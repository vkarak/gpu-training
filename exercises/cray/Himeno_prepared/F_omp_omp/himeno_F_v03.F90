...
!$omp target data map(alloc:a,b,c,p,wrk1,bnd,wrk2)
   omega = 0.8d0
   imax = mimax-1
   jmax = mjmax-1
   kmax = mkmax-1
...
!$omp target teams distribute default(shared) private(i,j,k)
   DO k = 1,mkmax
    DO j = 1,mjmax
     DO i = 1,mimax
      a(i,j,k,1) = 0d0
...
!$omp target teams distribute private(i,j,k)
   DO k = 1,kmax
    DO j = 1,jmax
     DO i = 1,imax
      a(i,j,k,1) = 1d0
...
!$omp target data map(tofrom:p) map(to:a,b,c,bnd,wrk1) map(alloc:wrk2)
...
!$omp&  private(i,j,k,s0,ss) reduction(+:gosa1) &
    DO k = 2,kmax-1
     DO j = 2,jmax-1
      DO i = 2,imax-1
