...
#pragma omp target data map(alloc:a,b,c,p,bnd,wrk1,wrk2)
{
  omega = 0.8;
  imax = MIMAX-1;
  jmax = MJMAX-1;
...
#pragma omp target data map(to:a,b,c,p,bnd,wrk1) \
  map(tofrom:p) map(alloc:wrk2)
  {
...
#pragma omp target teams distribute reduction(+:gosa1)
    for(k=1 ; k<kmax-1 ; ++k)
      for(j=1 ; j<jmax-1 ; ++j)
        for(i=1 ; i<imax-1 ; ++i){
