...
!$omp target data map(alloc:a,b,c,p,bnd,wrk1,wrk2) 
   omega = 0.8d0
   imax = mimax-1
   jmax = mjmax-1
   kmax = mkmax-1
...
!$omp target data map(to:a,b,c,bnd,wrk1) &
!$omp& map(tofrom:p) map(alloc:wrk2)
...
!$omp target teams distribute reduction(+:gosa1) &
!$omp thread_limit(NTPB)
    DO k = 2,kmax-1
     DO j = 2,jmax-1
      DO i = 2,imax-1
